...
/** RX buffer for USB */
uint8_t RX_Buffer[NUMBER_OF_CDC][APP_RX_DATA_SIZE] = {0};

/** TX buffer for USB, RX buffer for UART */
uint8_t TX_Buffer[NUMBER_OF_CDC][APP_TX_DATA_SIZE] = {0};

USBD_CDC_ACM_LineCodingTypeDef Line_Coding[NUMBER_OF_CDC];

uint32_t Write_Index[NUMBER_OF_CDC] = {0}; /* keep track of received data over UART */
uint32_t Read_Index[NUMBER_OF_CDC] = {0};  /* keep track of sent data to USB */
...
#define CDC_RX_BUFFER_SIZE 4096  // giống APP_RX_DATA_SIZE

uint8_t CDC_RX_Buffer[NUMBER_OF_CDC][CDC_RX_BUFFER_SIZE] = {0};
...
uint8_t process_sendframe[HID_FRAME_BUFFER_SIZE] = {0};
...
////	if (HAL_UART_Transmit_DMA(CDC_CH_To_UART_Handle(cdc_ch), Buf, *Len) != HAL_OK)
////	{
////	  HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_6);  // bật LED báo lỗi DMA
////	}
// // HAL_UART_Transmit_DMA(CDC_CH_To_UART_Handle(cdc_ch), Buf, *Len);
